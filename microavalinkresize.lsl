...
// Special thanks to:
// GreySoul Darkfury for making me notice that Maestro's version didn't have a menu.
...
Menu()
{
    ...
    llDialog(llGetOwner(),"Max scale: "+(string)max_scale+"\nMin scale: "+(string)min_scale+"\n \nCurrent scale: "+
        (string)cur_scale,["-0.01","-0.05","MIN SIZE","+0.01","+0.05","MAX SIZE","-0.10","-0.25","RESTORE","+0.10","+0.25","DELETE..."],menuChan);
}
